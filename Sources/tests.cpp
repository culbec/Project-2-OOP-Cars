...
void repositoryTests() {
	...
	try {
		...
		modifiedCar = carRepository.modifyCar(badCar);
		assert(false);
	}
	catch (RepositoryException& rE) {
		assert(rE.getMessage().size() > 0);
	}
	
	try {
		deletedCar = carRepository.deleteCar("VS48TUT");
		assert(false);
	}
	catch (const RepositoryException&) {
		assert(true);
...
void validatorTests() {
	...
	assert(carValid.validateRegNumber(car1) == true);
	carRepo.addCar(car1);

	assert(carValid.validateModel(car2) == true);
	carRepo.addCar(car2);

	assert(carValid.validateCar(car3, carRepo) == true);
	carRepo.addCar(car3);

	assert(carValid.validateType(car3) == true);
	...
	try {
		carValid.validateRegNumber(badCarRegNumber1);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber2);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber3);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber4);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber5);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber6);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }
	...
	try {
		carValid.validateRegNumber(badCarRegNumber8);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber9);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber10);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber11);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber12);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateRegNumber(badCarRegNumber13);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }
	...
	try {
		carValid.validateProducer(badCarProducer1);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateProducer(badCarProducer2);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }

	try {
		carValid.validateProducer(badCarProducer3);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }
	...
	try {
		carValid.validateModel(badCarModel);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }
	...
	try {
		carValid.validateType(badCarType);
		assert(false);
	}
	catch (const ValidatorException&) { assert(true); }
...
void serviceTests() {
	// initializam un service si verificam daca se initializeaza
	Service carService;
	
	assert(carService.getRepository().getSize() == 0);
	...
	Car wasModified = Car(carService.getRepository().getCars().at(0));
	...
	catch (ServiceException& sE) { assert(sE.getMessage().size() > 0); }
	...
	carList filterProducer = carService.filter("Mercedes", carService.compareByProducer);
	assert(filterProducer.size() == 3);

	filterProducer = carService.filter("Skoda", carService.compareByProducer);
	assert(filterProducer.size() == 1);

	// filtram si dupa un producator care nu exista

	try {
		carService.filter("Peugeot", carService.compareByProducer);
		assert(false);
	}
	catch (const ServiceException&) { assert(true); }

	carList filterType = carService.filter("Sport+", carService.compareByType);
	assert(filterType.size() == 1);

	filterType = carService.filter("Combi", carService.compareByType);
	assert(filterType.size() == 1);

	// filtram si dupa un tip care nu exista
	try {
		carService.filter("Sport", carService.compareByType);
		assert(false);
	}
	catch (const ServiceException&) { assert(true); }
